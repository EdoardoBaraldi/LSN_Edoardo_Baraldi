...
int main() { 
  ...
  for(int iblk=1; iblk <= nblk; iblk++) //Simulation
  {
   ...
   for(int istep=1; istep <= nstep; istep++)
   {
    Move();
    Measure();
    Accumulate(istep,iblk); //Update block averages
    if(istep%10 == 0){
...
//mesure of g(r)
  width_bin = (box/2.)/double(N_bins); //width of each bin
  
  for(int i=0; i<N_bins; i ++) delta_V[i] = 4.*M_PI/3. * (pow(width_bin*double(i + 1),3.) - pow(width_bin*double(i),3));
//update n_props 
  n_props= 5 + N_bins;
  
//tails corrections
  ene_corr = (8.*M_PI*rho*(1./(9.*pow(rcut,9)) - 1./(3.*pow(rcut,3))) * (double)npart);
  press_corr = (32.*M_PI*rho*(double)npart*(1./(9.*pow(rcut,9)) - 1./(6.*pow(rcut,3)))/vol);
  cout << "Tail correction for the potential energy = " << ene_corr << endl;
  cout << "Tail correction for the virial           = " << press_corr << endl << endl;
...
void Measure() //Properties measurement
{
  ...
  double vij, wij;
  double dx, dy, dz, dr;
  double g[N_bins]; //vector of variables to put in each bin of the hystogram
...
// distance i-j in pbc
  ...
  walker[iv] = 4.0 * v + ene_corr; // Potential energy with tail correction
  walker[ik] = kin; // Kinetic energy
  walker[it] = (2.0 / 3.0) * kin/(double)npart; // Temperature
  walker[ie] = 4.0 * v + kin;  // Total energy;
  walker[ip] = rho * (walker[it]) + (16./vol) * (w) + press_corr; //Pressure  with tail correction
  
  //update the hystogram
...
void Accumulate(int istep, int iblk) //Update block averages
{
  if(equilib || acceptance || U_on_N) {
   for(int i=0; i<n_props; ++i)
   {
...
void Averages(int iblk) //Print results for current block
{
    ...
    */
    ...
    ofstream Epot, Ekin, Etot, Temp, Pres, Isto, Isto_Final;
    
    if(acceptance) {
     ofstream Acc;
     Acc.open("acceptance_folder/gas_acc_"+to_string(delta)+".dat",ios::app);
     //Acceptance
     Acc << setw(wd) << iblk <<  setw(wd) << accepted/attempted << setw(wd) << ACC/(double)iblk << setw(wd) << delta << endl;
     Acc.close();
    }
    ...
    if(U_on_N || equilib) {
     ...
     if(U_on_N) { Epot.open("correlation_folder/gas_correlation.dat",ios::app);}
     if(equilib) { Epot.open("equilib_folder/solid_equilibration.dat",ios::app);}
     ...
     cout << "Acceptance rate " << accepted/attempted << endl << endl;
     cout << "----------------------------" << endl << endl;	
     Epot.open("output_folder/"+to_string(iNVET)+"gas_epot.dat",ios::app);
     //Etot.open("output_folder/"+to_string(iNVET)+"gas_etot.dat",ios::app);
     Pres.open("output_folder/"+to_string(iNVET)+"gas_pres.dat",ios::app);
     Isto.open("output_folder/"+to_string(iNVET)+"gas_isto.dat",ios::app);
     Isto_Final.open("output_folder/"+to_string(iNVET)+"gas_isto_final.dat",ios::app);
...
//Pressure 
     ...
     for (int i=0; i < N_bins; i++) {
      Isto << iblk << "," << i << "," << glob_av[i+5]/(double)iblk << "," << err_g[i] << endl;
      if(iblk == nblk) {
       Isto_Final << i*width_bin << "," << glob_av[i+5]/(double)iblk << "," << err_g[i] << endl;
      }
     }
    }
