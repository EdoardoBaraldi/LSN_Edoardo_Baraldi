...
int main() {
 if(equilib) {   //Check if the equilib flag is set to true
  new_temp_eq = 1.2; // initialize the new temperature 
  vector<double> temp_final; // declare a variable to store the mean temperature after n blocks
  double mean_temp; // mean teamp after n blocks
  do { 
   Input(); //Inizialization
   nblk = 150000; // set the number of blocks 
   nstep = 1; // set the number of step to 1
   for(int iblk=1; iblk <= nblk; iblk++) //Simulation
   {
    ...
    temp_final.push_back(stima_temp); // store the final temperature in the vector
   }
   // compute the mean temperature over all the blocks
   mean_temp = accumulate(temp_final.begin(), temp_final.end(), 0.0) / temp_final.size();
   // correct the new temperature based on the difference between the mean temperature and the reference temperature
   new_temp_eq += (ref_temp - mean_temp);
   cout << "Mean temeprature after 10^4 step: " << mean_temp << endl;
   cout << "New temparature is: " << new_temp_eq << endl << endl;
   cout << "----------------------------" << endl << endl;
   temp_final.clear();
   index ++;
  } while ( abs(ref_temp - mean_temp) > 0.002 );
 } 
...
// distance i-j in pbc
  ...
  walker[ip] = rho * (walker[it]) + 16./vol * w; //Pressure  
...
{
 if( (!equilib) && (walker[it] <= (ref_temp + 0.1*ref_temp)) && (walker[it] >= (ref_temp - 0.1*ref_temp)) ) {
   for(int i=0; i<n_props; ++i)
   {
...
void Averages(int iblk) //Print results for current block
{
   ...
   if(!equilib) {	
    ...
    if(equilib) {
     ofstream Equil;
     Equil.open("equilib_folder/gas_equilib_test" + to_string(index) +".dat", ios::app);
     Equil << setw(wd) << iblk <<  setw(wd) << stima_temp << setw(wd) << stima_kin << setw(wd) << stima_etot << endl;
     // block number ... temp in i-th block ... ekin in i-th block ... total e in i-th block
     Equil.close();
    }
    
    else{ 
     ofstream Epot, Ekin, Etot, Temp, Pres;
     Epot.open("output_folder/gas_epot.dat",ios::app);
     Ekin.open("output_folder/gas_ekin.dat",ios::app);
     Temp.open("output_folder/gas_temp.dat",ios::app);
     Etot.open("output_folder/gas_etot.dat",ios::app);
     Pres.open("output_folder/gas_pres.dat",ios::app);
